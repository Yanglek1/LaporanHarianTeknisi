...
const NAV_ITEMS = [
  ...
  { route: 'psb', label: 'PSB', icon: ICON.list },
...
const SCREEN_TITLES = {
  ...
  instalasi: ['Instalasi', 'Sedang berjalan'],
...
function screenDashboard() {
  ...
  return `
    ...
    <div class="stat-grid">
      <button class="stat-card" style="text-align:left; cursor:pointer;" id="qDiagnosa">
      ...
      <button class="stat-card" style="text-align:left; cursor:pointer;" id="qLaporan">
...
function wireDashboard() {
  ...
  if (lb) lb.onclick = () => { state.psbFilter = { query: '', chip: 'problem' }; state.history = []; goTo('psb', {}, { replace: true }); };
}
...
function screenPsb() {
  ...
  return `
    ...
    <div class="filter-chips">
      ${PSB_FILTER_CHIPS.map(c => `<div class="chip ${state.psbFilter.chip === c.key ? 'active' : ''}" data-chip="${c.key}">${c.label}</div>`).join('')}
    </div>
...
function psbCardHtml(p) {
  return `
    <div class="card psb-card" data-psb="${p.id}">
      <div class="psb-top">
        <div>
...
function wirePsb() {
  ...
  document.querySelectorAll('.psb-card').forEach(c => c.onclick = () => goTo('psb-detail', { psbId: c.dataset.psb }));
}

/* ---------------------------------------------------------------------- */
/* SCREEN: Instalasi tab (in-progress drafts)                             */
/* ---------------------------------------------------------------------- */
function screenInstalasi() {
  const drafts = DB.get('installations', []).filter(i => i.status === 'draft');
  const inProgressPsb = DB.get('psb', []).filter(p => p.status === 'progress');
  return `
    <div class="section-title">Draft Belum Selesai</div>
    ${drafts.length ? drafts.map(d => `
      <div class="card psb-card" data-draft="${d.id}">
        <div class="psb-top">
      ...
      </div>`).join('') : emptyState('Belum ada draft', 'Draft form instalasi yang belum selesai akan muncul di sini.')}

    <div class="section-title">PSB Dalam Proses</div>
    ${inProgressPsb.length ? inProgressPsb.map(psbCardHtml).join('') : emptyState('Tidak ada yang berjalan', 'Belum ada PSB berstatus dalam proses.')}
  `;
}
...
function screenProfil() {
  ...
  return `
    ...
    <button class="btn btn-danger" style="margin-top:16px;" id="btnLogout">${ICON.logout} Keluar</button>
  `;
}
function menuRow(icon, label, id) {
  return `<div class="row-between" id="${id}" style="padding:13px 0; border-bottom:1px solid var(--border); cursor:pointer;">
    <div class="row-between" style="gap:10px; justify-content:flex-start;"><span class="muted">${icon}</span><span class="small">${label}</span></div>
...
function renderView() {
  ...
  switch (state.route) {
    ...
    case 'psb': view.innerHTML = screenPsb(); wirePsb(); break;
...
function render() {
  ...
  renderBottomNav();
  renderView();
  // FAB only on PSB / Instalasi tabs, shortcut to nearest pending PSB
  const oldFab = document.getElementById('fabNew');
  if (oldFab) oldFab.remove();
  if (session() && (state.route === 'psb' || state.route === 'instalasi')) {
    const fab = document.createElement('button');
    fab.className = 'fab'; fab.id = 'fabNew';
    fab.innerHTML = ICON.plus + 'Instalasi';
    fab.onclick = () => {
