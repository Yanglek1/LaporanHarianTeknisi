...
/* ---- Pengujian jaringan nyata ------------------------------------------
   Memakai endpoint publik infrastruktur Cloudflare Speed Test (domain yang
   sama yang dipakai speed.cloudflare.com dan npm package resmi
   @cloudflare/speedtest, dan memang didesain untuk dipakai lintas-domain):
     /meta   -> info klien: IP publik, ISP (ASN org), lokasi data center
     /__down -> unduh N byte, dipakai untuk uji ping (0 byte) & download
     /__up   -> unggah data, dipakai untuk uji upload
   Semua pengukuran dihitung dari waktu tempuh permintaan sungguhan — bukan
   angka acak — sehingga hasilnya benar-benar mencerminkan koneksi HP/PC
   yang sedang dipakai saat itu. */
const CF_BASE = 'https://speed.cloudflare.com';
const COLO_CITY = {
  CGK: 'Jakarta, Indonesia', DPS: 'Denpasar, Indonesia', SIN: 'Singapura',
  KUL: 'Kuala Lumpur, Malaysia', HKG: 'Hong Kong', BKK: 'Bangkok, Thailand',
  NRT: 'Tokyo, Jepang', KIX: 'Osaka, Jepang', ICN: 'Seoul, Korea Selatan',
  TPE: 'Taipei, Taiwan', SYD: 'Sydney, Australia', MEL: 'Melbourne, Australia',
  LAX: 'Los Angeles, AS', SJC: 'San Jose, AS', SEA: 'Seattle, AS',
  ORD: 'Chicago, AS', DFW: 'Dallas, AS', IAD: 'Washington DC, AS',
  LHR: 'London, Inggris', AMS: 'Amsterdam, Belanda', FRA: 'Frankfurt, Jerman',
  CDG: 'Paris, Prancis', MAA: 'Chennai, India', BOM: 'Mumbai, India',
  BLR: 'Bengaluru, India', DEL: 'Delhi, India', DXB: 'Dubai, UEA',
};

async function fetchNetMeta() {
  const res = await fetch(`${CF_BASE}/meta`, { cache: 'no-store' });
  if (!res.ok) throw new Error('Tidak dapat mengambil info jaringan');
  return res.json();
}

// Ping diukur dari waktu tempuh beberapa permintaan 0-byte berturut-turut;
// jitter dari rata-rata selisih tiap sampel; packet loss dari sampel gagal.
async function measurePing(samples = 8, onSample) {
  const timings = [];
  let failures = 0;
  for (let i = 0; i < samples; i++) {
    const t0 = performance.now();
    try {
      await fetch(`${CF_BASE}/__down?bytes=0&cb=${Math.random()}`, { cache: 'no-store' });
      const t = performance.now() - t0;
      timings.push(t);
      onSample && onSample(t);
    } catch (e) { failures++; }
  }
  if (!timings.length) throw new Error('Gagal mengukur ping');
  const ping = Math.min(...timings);
  const avg = timings.reduce((a, b) => a + b, 0) / timings.length;
  const jitter = timings.reduce((a, b) => a + Math.abs(b - avg), 0) / timings.length;
  const packetLoss = (failures / samples) * 100;
  return { ping: +ping.toFixed(0), jitter: +jitter.toFixed(1), packetLoss: +packetLoss.toFixed(1) };
}

// Download diukur dengan membaca stream sungguhan secara bertahap (bukan
// menunggu selesai lalu bagi total), sehingga kecepatan sesaat bisa
// ditampilkan live di gauge selagi data mengalir masuk.
async function measureDownload(onProgress, maxDurationMs = 7000) {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), maxDurationMs);
  const t0 = performance.now();
  let received = 0, lastTick = t0, lastBytes = 0;
  try {
    const res = await fetch(`${CF_BASE}/__down?bytes=200000000&cb=${Math.random()}`, { signal: controller.signal, cache: 'no-store' });
    if (!res.ok || !res.body) throw new Error('Server pengujian tidak merespons');
    ...
    while (true) {
      ...
      if (now - lastTick > 120) {
        const instMbps = ((received - lastBytes) * 8) / 1e6 / ((now - lastTick) / 1000);
        onProgress(instMbps);
  ...
  if (received < 50000) throw new Error('Data unduhan terlalu sedikit untuk diukur');
  return { mbps: +((received * 8) / 1e6 / totalSeconds).toFixed(1), bytes: received, seconds: +totalSeconds.toFixed(2) };
}

// Upload diukur via XHR (fetch belum konsisten mendukung progress unggah di
// semua browser) mengirim payload biner ke endpoint /__up, dilacak live
// lewat xhr.upload.onprogress.
function measureUpload(onProgress, sizeBytes = 14000000, maxDurationMs = 7000) {
  return new Promise((resolve, reject) => {
    const blob = new Blob([new Uint8Array(sizeBytes)]);
    const xhr = new XMLHttpRequest();
    const t0 = performance.now();
    let lastTick = t0, lastLoaded = 0, settled = false;
    const timeout = setTimeout(() => { if (!settled) xhr.abort(); }, maxDurationMs);
    xhr.open('POST', `${CF_BASE}/__up`, true);
    xhr.upload.onprogress = (e) => {
      const now = performance.now();
      if (now - lastTick > 120 && e.loaded > lastLoaded) {
        onProgress(((e.loaded - lastLoaded) * 8) / 1e6 / ((now - lastTick) / 1000));
    ...
    xhr.onloadend = () => {
      ...
      const totalSeconds = (performance.now() - t0) / 1000;
      const bytes = Math.max(lastLoaded, xhr.status ? sizeBytes : lastLoaded);
      if (bytes < 50000) { reject(new Error('Data unggahan terlalu sedikit untuk diukur')); return; }
      resolve({ mbps: +((bytes * 8) / 1e6 / totalSeconds).toFixed(1), bytes, seconds: +totalSeconds.toFixed(2) });
    };
    xhr.onerror = () => { clearTimeout(timeout); reject(new Error('Gagal mengunggah data uji')); };
    xhr.send(blob);
  });
}

// Bucket skala gauge — membesar mengikuti nilai puncak yang terlihat supaya
// jarum tidak "mentok" di ujung kanan saat koneksi kencang.
function pickGaugeMax(v) {
  const buckets = [10, 25, 50, 100, 250, 500, 1000, 2000];
  return buckets.find(b => v <= b * 0.85) || v * 1.3;
}
function makeSmoother(alpha = 0.35) {
  let v = null;
  return (x) => { v = v === null ? x : v * (1 - alpha) + x * alpha; return v; };
}

function screenSpeedtest() {
  const history = DB.get('speedtests', []).sort((a, b) => b.timestamp - a.timestamp).slice(0, 5);
  return `
    <div class="banner info">${ICON.gauge}<div>Pengujian mengukur kecepatan sungguhan menggunakan infrastruktur Cloudflare Speed Test dari koneksi HP/PC ini. Pastikan terhubung ke internet.</div></div>
    ...
    <div class="card" style="padding:4px 16px;">
      ${history.map(h => `
        <div class="row-between" style="padding:11px 0; border-bottom:1px solid var(--border);">
          <div><div class="small">${fmtDateTime(h.timestamp)}</div><div class="small muted mono" style="margin-top:2px;">${h.provider || '-'} · ${h.server || '-'}</div></div>
...
async function runSpeedtest() {
  ...
  setGaugeValue('mainGauge', 0, 50, 'MS', 0);
  setGaugePhase('mainGauge', 'MENDETEKSI JARINGAN…');
  serverInfoEl.innerHTML = `<div class="small muted center" style="margin-bottom:10px;">Mendeteksi provider &amp; server terdekat…</div>`;

  let meta;
  try {
    meta = await fetchNetMeta();
  } catch (e) {
    setGaugePhase('mainGauge', 'GAGAL');
    serverInfoEl.innerHTML = `<div class="banner danger">${ICON.alert}<div>Tidak dapat terhubung ke server pengujian. Periksa koneksi internet Anda, lalu coba lagi.</div></div>`;
    btn.disabled = false; btn.textContent = 'Coba Lagi';
    return;
  }

  const provider = meta.asOrganization || meta.asn || 'Tidak diketahui';
  const colo = meta.colo || '-';
  const serverCity = COLO_CITY[colo] || 'Lokasi tidak dikenali';
  const server = `Cloudflare · ${colo}`;
  serverInfoEl.innerHTML = `
    <div class="card" style="margin-bottom:14px;">
      ${infoRow('Provider (ISP)', provider)}
      ${infoRow('Alamat IP', meta.clientIp || '-', true)}
      ${infoRow('Server Pengujian', server, true)}
      ${infoRow('Lokasi Server', serverCity)}
    </div>`;

  try {
    setGaugePhase('mainGauge', 'MENGUJI PING…');
    const pingSmooth = makeSmoother(0.5);
    const pingResult = await measurePing(8, (t) => setGaugeValue('mainGauge', pingSmooth(t), pickGaugeMax(t), 'MS', 0));
    setGaugeValue('mainGauge', pingResult.ping, pickGaugeMax(pingResult.ping), 'MS', 0);

    setGaugePhase('mainGauge', 'MENGUJI DOWNLOAD…');
    const downSmooth = makeSmoother();
    let peakDown = 10;
    const downResult = await measureDownload((inst) => {
      const sm = downSmooth(inst);
      peakDown = Math.max(peakDown, sm);
      setGaugeValue('mainGauge', Math.max(0, sm), pickGaugeMax(peakDown), 'MBPS', 1);
    });
    setGaugeValue('mainGauge', downResult.mbps, pickGaugeMax(Math.max(peakDown, downResult.mbps)), 'MBPS', 1);

    setGaugePhase('mainGauge', 'MENGUJI UPLOAD…');
    const upSmooth = makeSmoother();
    let peakUp = 10;
    const upResult = await measureUpload((inst) => {
      const sm = upSmooth(inst);
      peakUp = Math.max(peakUp, sm);
      setGaugeValue('mainGauge', Math.max(0, sm), pickGaugeMax(peakUp), 'MBPS', 1);
    });
    setGaugeValue('mainGauge', upResult.mbps, pickGaugeMax(Math.max(peakUp, upResult.mbps)), 'MBPS', 1);

    setGaugePhase('mainGauge', 'SELESAI');
    finishSpeedtest({
      ping: pingResult.ping, jitter: pingResult.jitter, packetLoss: pingResult.packetLoss,
      download: downResult.mbps, upload: upResult.mbps,
      provider, ip: meta.clientIp || '-', server, serverCity,
    });
    btn.disabled = false; btn.textContent = 'Uji Ulang';
  } catch (e) {
    setGaugePhase('mainGauge', 'GAGAL');
    toast('Pengujian gagal: ' + e.message, 'danger');
    btn.disabled = false; btn.textContent = 'Coba Lagi';
  }
}
...
function finishSpeedtest(r) {
  ...
  document.getElementById('speedResult').innerHTML = `
    ...
    <div class="card" style="margin-top:10px;">
      ${infoRow('Packet Loss', r.packetLoss + '%', true)}
      ${infoRow('Provider (ISP)', r.provider)}
      ${infoRow('Server Pengujian', `${r.server} · ${r.serverCity}`)}
      ${infoRow('Alamat IP', r.ip, true)}
      <div class="divider"></div>
    ...
    <div class="small muted center" style="margin-top:10px;">Diukur langsung dari koneksi perangkat ini via infrastruktur Cloudflare Speed Test.</div>
  `;
}
